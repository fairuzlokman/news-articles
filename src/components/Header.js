...
const Header = () => {
	...
	return (
		<div>
			...
			{isSearching && (
				<div className="absolute z-40 flex flex-wrap justify-center w-full gap-1 p-1 bg-white shadow-md lg:hidden">
					{searchSuggestions.map((item, index) => (
						<Link
							key={index}
							href={item}
							className="px-3 py-2 text-sm hover:underline hover:underline-offset-2"
						>
							{item}
						</Link>
					))}
